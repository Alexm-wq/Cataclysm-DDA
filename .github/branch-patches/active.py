...
# Shared dropdown helper for the right-click Change context menu.
replace_once(
    "src/game.cpp",
...
# Make shortcut storage dynamic.  Five slots remain the minimum/base layout, but
# extra assignments are preserved instead of being truncated on load.
replace_once(
    "src/uistate.h",
...
# Keep the existing fixed base/safemode controls and add stable heap-owned controls
# for dynamically growing extra slots.  unique_ptr keeps SDL overlay owner addresses
# stable even when the vector itself grows.
replace_once(
    "src/game.h",
...
# Dynamic slot model: base five, then six-high columns expanding to the left,
# always exposing one trailing empty slot after the highest assigned shortcut.
replace_once(
    "src/game.cpp",
...
# Pixel and curses geometry for extra columns.  Slot 5 begins immediately left of
# the fixed safemode cell, slot 6 goes above it, and every six extra slots start a
# fresh column farther left at the bottom.
insert_before(
    "src/game.cpp",
...
# Small shared-helper right-click menu.  It is a real modal ui_adaptor so map/HUD
# redraws cannot race the dropdown.  The dropdown itself owns hit testing and
...
# Close dynamically allocated controls whenever the core palette is unavailable.
replace_once(
    "src/game.cpp",
...
# Draw dynamic extra slots after the fixed base stack.  Existing five-slot visual
# layout and fixed safemode button are unchanged.
insert_before(
    "src/game.cpp",
...
# Collapse closes extras as well.  This second occurrence is intentionally scoped
# by the immediately following tooltip reset block.
replace_once(
    "src/game.cpp",
    '''    } else {\n        for( ui_icon_button &button : safemode_corner_buttons ) {\n            button.close();\n        }\n        safemode_corner_tooltip.reset();\n    }\n''',
    '''    } else {\n        for( ui_icon_button &button : safemode_corner_buttons ) {\n            button.close();\n        }\n        for( const std::unique_ptr<ui_icon_button> &button : safemode_corner_extra_buttons ) {\n            button->close();\n        }\n        safemode_corner_tooltip.reset();\n    }\n'''
...
# Hover state follows dynamic controls too.
replace_once(
    "src/game.cpp",
...
# Rewrite HUD click resolution so both mouse buttons are consumed by controls.
# Secondary click on assignable slots opens helper-owned Change -> picker; secondary
# click on the launcher/safemode cell is consumed without reaching terrain.
...
if text.count(start) != 1 or text.count(end) < 1:
    raise SystemExit("safemode mouse action markers not found")
        ...
        const std::optional<point> &pixel_p, const bool secondary )
{
    ...
    // Secondary clicks that did not hit the HUD should continue to normal terrain handling.
    if( secondary ) {
        return ACTION_NULL;
    }
...
# Route right-click through the HUD before terrain target resolution, just like left-click.
replace_once(
