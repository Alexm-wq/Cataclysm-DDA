...
def replace_span(text, start_marker, end_marker, replacement, label, keep_end=True):
...
multi_path = "src/ui_helpers/models/multiselect_filter.h"
...
# std::move is now used directly in this header.
multi = replace_once(
    multi,
    '#include <set>\n#include <vector>\n',
    '#include <set>\n#include <utility>\n#include <vector>\n',
    "multiselect utility include",
)
write(multi_path, multi)


tree_path = Path("src/ui_helpers/controls/tree_dropdown.h")
if tree_path.exists():
    raise SystemExit("tree_dropdown.h already exists; refusing to overwrite an unexpected helper")
tree_path.write_text(r'''#pragma once
...
struct ui_tree_dropdown_style {
    ...
    nc_color expander = c_light_cyan;
    int indent = 2;
};
...
# Persist the new presentation-only filter/sort/scope state explicitly.
...
craft = replace_once(
    ...
    "crafting tree dropdown include",
)
...
# Replace old sidebar-centric state with filter/scope/sort state.
...
# Startup: build concrete category leaves once and restore the persisted multi-selection.
browser = replace_span(
    browser,
    ...
    if( !restored_multi ) {
        ...
        if( old_category_found != category_leaf_keys.end() ) {
            if( old_subcategory == "CSC_ALL" ) {
                ...
                const std::string key = leaf_key( old_category, old_subcategory );
                state.category_selection.set( key, true );
            }
        } else {
...
''',
    "crafting startup and category leaves",
)

# Restore selected recipe / goto recipe without scalar category state.
browser = replace_span(
    browser,
...
''',
    "crafting selected recipe restore",
)

# Replace sidebar controls with header controls/dropdowns.
browser = replace_span(
    browser,
...
''',
    ...
    catacurses::window w_recipes;
''',
    "remove crafting sidebar window",
)
browser = replace_once(
    browser,
    '''    int header_height = 3;
...
''',
    ...
    int body_height = 0;
''',
    "crafting header height default",
)

# Persist the new models and supply semantic helpers used by both drawing and input.
browser = replace_span(
    browser,
    ...
    const auto recipe_matches_categories = [&]( const recipe * rec ) {
        ...
        return state.category_selection.contains( leaf_key( rec->category.str(), rec->subcategory ) );
    };
    ...
    const auto persist_state = [&]() {
        ...
        // Keep the old scalar fields meaningful for backward compatibility.
...
''',
    "crafting semantic helpers and persistence",
)

# Responsive shell: two content panes, header controls instead of a permanent sidebar.
...
if resize_start < 0 or resize_end < 0:
    raise SystemExit("crafting resize block not found")
...
# Redraw prefix through the old sidebar is replaced wholesale.
redraw_start = browser.find("    ui.on_redraw( [&]( ui_adaptor & ui ) {")
recipes_marker = "        if( draw_recipes ) {"
recipes_pos = browser.find(recipes_marker, redraw_start)
if redraw_start < 0 or recipes_pos < 0:
    raise SystemExit("crafting redraw prefix/sidebar block not found")
new_redraw_prefix = r'''    ui.on_redraw( [&]( ui_adaptor & ui ) {
        recipe_hits.clear();
        ...
        if( draw_recipes ) {'''
browser = browser[:redraw_start] + new_redraw_prefix + browser[recipes_pos + len(recipes_marker):]

# Recipe pane scope line follows the new independent category/view models.
old_scope_re = re.compile(r'''            std::string scope = state\.search_query\.empty\(\) \?.*?            trim_and_print\( w_recipes, point\( 1, 1 \),''', re.S)
match = old_scope_re.search(browser)
if not match:
    raise SystemExit("recipe scope display block not found")
new_scope = r'''            std::string scope = scope_summary() + " | " + category_summary();
            ...
            trim_and_print( w_recipes, point( 1, 1 ),'''
browser = browser[:match.start()] + new_scope + browser[match.end():]

# Draw helper-owned dropdown overlays last, above both panes.
old_redraw_end = '''        wnoutrefresh( w_actions );
    } );'''
new_redraw_end = r'''        wnoutrefresh( w_actions );
...
browser = replace_once(browser, old_redraw_end, new_redraw_end, "crafting header dropdown draw")

# Candidate source is now All + semantic category/view filtering, regardless of search.
candidate_start = browser.find("        std::vector<const recipe *> candidates;")
candidate_sort = browser.find("        if( state.selected_subcategory != \"CSC_*_RECENT\"", candidate_start)
if candidate_start < 0 or candidate_sort < 0:
    raise SystemExit("crafting candidate/sort block markers missing")
new_candidates = r'''        std::vector<const recipe *> candidates;
        if( !state.search_query.empty() ) {
...
'''
browser = browser[:candidate_start] + new_candidates + browser[candidate_sort:]

# Replace the old single sorting policy with a dropdown-selected policy.
...
if sort_start < 0 or sort_end < 0:
    raise SystemExit("crafting old sort block not found")
new_sort = r'''        if( !( state.scope == crafting_scope::recent && state.sort == crafting_sort::recommended &&
            ...
            [&]( const recipe * a, const recipe * b ) {
                ...
                switch( state.sort ) {
                    ...
                    case crafting_sort::unread:
                        ...
                        // fall through to the normal recommended tie-breaks
                    case crafting_sort::recommended:
                    default:
                        if( a_avail.can_craft != b_avail.can_craft ) {
                            return a_avail.can_craft;
...
'''
browser = browser[:sort_start] + new_sort + browser[sort_end:]

# Expanded nested children must still obey the active category leaf selection.
browser = replace_once(
    browser,
...
''',
    '''            const availability &rec_avail = availability_cache->at( rec );
            ...
            if( state.filters.contains( crafting_filter::craftable ) &&
''',
    "expanded recipe category filter",
)

browser = browser.replace("expand_recipes( candidates, indent, *availability_cache, *crafter, state.unread_first,",
                          "expand_recipes( candidates, indent, *availability_cache, *crafter,\n                        state.sort == crafting_sort::unread,")

# Input positions: dropdowns operate in full-screen coordinates.
browser = replace_once(
    browser,
...
''',
    '''        const std::optional<point> header_pos = local_mouse( w_header );
        ...
        const std::optional<point> recipes_pos = local_mouse( w_recipes );
''',
    "crafting input positions",
)

# Helper-owned header dropdowns get first refusal while open.
context_marker = "        if( state.context_open ) {"
context_at = browser.find(context_marker)
if context_at < 0:
    raise SystemExit("crafting context input marker missing")
header_input = r'''        if( state.open_header_menu == "CATEGORIES" ) {
...
'''
browser = browser[:context_at] + header_input + browser[context_at:]

# Replace mouse-move routing wholesale.
...
if mouse_start < 0 or select_start < 0:
    raise SystemExit("crafting mouse/select blocks not found")
new_mouse = r'''        if( action == "MOUSE_MOVE" ) {
...
'''
browser = browser[:mouse_start] + new_mouse + browser[select_start:]

# Replace SELECT through before SEC_SELECT.
...
if select_start < 0 or sec_start < 0:
    raise SystemExit("crafting select/sec-select bounds missing")
new_select = r'''        if( action == "SELECT" ) {
            bool handled = false;
            if( header_pos ) {
                ...
                if( result.type == ui_action_result_type::activated && result.entry ) {
                    ...
                    const std::string requested = id == "HEADER_CATEGORIES" ? "CATEGORIES" :
                                                  ...
                                                  id == "HEADER_VIEW" ? "VIEW" : "";
                    if( !requested.empty() ) {
                        const bool closing = state.open_header_menu == requested;
                        state.open_header_menu = closing ? std::string() : requested;
                        category_menu.close();
                        header_menu.close();
...
'''
browser = browser[:select_start] + new_select + browser[sec_start:]

# SEC_SELECT and scrolling no longer have a Categories pane/sidebar.
browser = browser.replace('''            if( ( !compact_layout ||
                  state.focused_pane == crafting_browser_pane::recipes ) && recipes_pos ) {''',
                          '''            if( ( !compact_layout ||
                  state.focused_pane == crafting_browser_pane::recipes ) && recipes_pos ) {''', 1)
scroll_start = browser.find('        if( action == "SCROLL_UP" || action == "SCROLL_DOWN" ) {')
index_marker = browser.find('        const int index = selected_index();', scroll_start)
if scroll_start < 0 or index_marker < 0:
    raise SystemExit("crafting scroll block bounds missing")
new_scroll = r'''        if( action == "SCROLL_UP" || action == "SCROLL_DOWN" ) {
...
'''
browser = browser[:scroll_start] + new_scroll + browser[index_marker:]

# Keyboard tab/category compatibility: cycling selects one whole category/subcategory.
nav_start = browser.find('        } else if( action == "PREV_TAB" || action == "NEXT_TAB" ) {')
filter_marker = browser.find('        } else if( action == "FILTER" ) {', nav_start)
if nav_start < 0 or filter_marker < 0:
    raise SystemExit("crafting category keyboard navigation block missing")
new_nav = r'''        } else if( action == "PREV_TAB" || action == "NEXT_TAB" ) {
...
'''
browser = browser[:nav_start] + new_nav + browser[filter_marker:]

# Special-view and unread-first legacy actions now target the new models.
browser = replace_once(
    browser,
...
''',
    '''        } else if( action == "TOGGLE_UNREAD_RECIPES_FIRST" &&
            ...
            recalc = true;
''',
    "unread sort compatibility",
)

# The browser should contain no old sidebar/scalar-category state after migration.
for forbidden in [
    ...
    if forbidden in browser:
        raise SystemExit(f"crafting migration incomplete; forbidden token remains: {forbidden}")
