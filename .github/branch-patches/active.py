...
def replace_once(path_str: str, old: str, new: str) -> None:
    ...
    if count != 1:
        raise SystemExit(f"expected exactly one block in {path_str}, found {count}: {old[:160]!r}")
    path.write_text(text.replace(old, new, 1), encoding="utf-8")


helper = Path("src/ui_helpers/controls/text_input_dialog.h")
if helper.exists():
    raise SystemExit("text_input_dialog.h already exists; refusing to overwrite")

helper.write_text(r'''#pragma once
...
Path("/tmp/branch_patch_commit_message").write_text(
    "Modernize vehicle rename and relabel dialogs\n", encoding="utf-8"
)
