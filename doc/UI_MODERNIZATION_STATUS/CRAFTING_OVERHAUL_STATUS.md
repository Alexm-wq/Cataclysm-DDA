...
- On the compass, solid tiles indicate blocked locations, red indicates danger, green indicates existing items (including carried/worn/wielded items on `You`), and gray indicates empty storage. In the destination list, green means usable and gray means unavailable, regardless of contents. Embedded inventory-name colors cannot override these colors. Storage size/capacity failures show a red `too small!` beside the gray name; other failures retain their native explanation. Capacity labels use native pocket failure codes without parsing translated messages, and normal partial-placement mechanics are unchanged.
