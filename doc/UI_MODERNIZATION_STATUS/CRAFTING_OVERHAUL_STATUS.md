...
- The recipe inspector shows a shared `ui_compass_grid` for nearby output storage when space permits. `Output…` (`o`) opens the same picker on compact layouts. The compass is anchored on the player, with `You` at its center even when another crafter is selected. In the picker, 1–9 select compass tiles, Up/Down/Enter select destinations, and Left/Right collapse/expand containers.
- Clicking a tile always opens its destination list, even if it has only ground or one usable storage option. Browsing a tile never commits a destination or closes the picker. Ground is not preselected when the recipe changes; normal placement stays in effect until the player chooses. A shared selection panel lists furniture surfaces/storage, vehicle cargo, portable containers, liquid tanks and kegs as applicable. Ground is a separate leaf, never a container or a tree root. Containers on bare ground appear beside it; contents of furniture, cargo and other containers nest under their actual storage parent. Furniture and the ground below it share the game's existing map stack; they are not presented as fictitious separate inventories.
- The player tile includes an expandable `You` inventory group. The shared `ui_selection_list` uses `ui_tree_model` for multiple levels of nesting, stable row identities and expansion. Clicking an arrow only expands/collapses; one click on an eligible container marks the candidate, and double-click, Enter or the unchanged `Use selected` button confirms that exact destination. Inventory groups only expand. Disabled outer containers can reveal eligible inner containers. Collapse and scrolling retain the selected destination; switching between tiles retains each list's expansion and scroll state, and reopening reveals the selected path.
- On the compass, solid tiles indicate blocked locations, red indicates danger, green indicates existing items (including carried/worn/wielded items on `You`), and gray indicates empty storage. In the destination list, green means usable and gray means unavailable, regardless of contents. Inventory groups such as `You` instead use green when they contain carried/worn/wielded items and gray when empty; they remain expandable groups, not selectable destinations. Embedded inventory-name colors cannot override these colors. Storage size/capacity failures show a red `too small!` beside the gray name; other failures retain their native explanation. Capacity labels use native pocket failure codes without parsing translated messages, and normal partial-placement mechanics are unchanged.
- The shared list renders one focus highlight for single-selection menus and clears stale hover on clicks/navigation/scrolling. Hover previews rows before a choice is made; after a click, the chosen row keeps its highlight while the pointer crosses other rows or moves to `Use selected`. Clicking another row or keyboard navigation explicitly moves the focus. Intentional multiple selections in refuel/siphon remain highlighted. Inline disabled hints and label-color overrides are provided by the helper rather than custom crafting controls.
...
- Regression coverage was added to `crafting_test.cpp` and `ui_helpers_test.cpp`, including separate ground, player occupancy, furniture/cargo/carried nesting, deep trees, stable selection across collapse, one focus highlight, double-click confirmation, size-vs-compatibility failures and no ground preselection. The available renderer-independent helper subset passed (12 cases, 126 assertions); affected sources/tests were syntax checked. Full game/control tests and Windows/TILES mouse/resize checks still require the normal game build.

Manual checks: click each compass direction, including a tile with only ground, and verify the picker stays open; hover/click different rows and check there is only one highlight, then move across other rows to `Use selected` and verify the clicked row remains highlighted; compare fitting, too-small and incompatible destinations; confirm single clicks do not close the picker, while double-click/Enter/`Use selected` do; verify the `You` label and center occupancy with only carried items and only ground items; expand several inventory/furniture/cargo/container levels; distinguish arrow clicks from destination selection; collapse a selected descendant without losing its destination; switch tiles and reopen the selected path; distinguish cargo from ground on one vehicle tile; craft a batch into limited storage; interrupt/save/load; remove a selected container; and resize between inspector and compact layouts. Recheck flat refuel/siphon/unload and vehicle relabel menus after shared list changes.
