...
**Status:** Build/Remove/Plan/Plans workspace implemented; in-game acceptance pending
...
- Shared Build/Remove/Plan/Plans toolbar and right-aligned Back control, with all four workflows active.
...
- Map clicks inspect when no catalog tool is selected. With a Build or Plan result selected, clicking applies that ghost at the chosen tile; disabled locations stay selected and explain why they cannot be used.
...
- Distant Build and Remove reserve ordinary unfinished work, walk to a reachable adjacent work tile, and continue through the normal `ACT_BUILD` lifecycle. The queued build activity uses the same indefinite progress lifetime as a directly assigned build, so it cannot expire after its first arrival tick.
- UI test mode consistently bypasses construction skills, tools, components, and darkness in both one-shot and multi-construction paths while still enforcing physical tile validity and movement.
- Plan mode creates persistent single-tile construction intent without consuming materials or creating `partial_con`. Same-plan placement is a no-op; replacement is explicit; unrelated unfinished construction is never overwritten.
- The plan adapter stores intent through existing `CONSTRUCTION_BLUEPRINT` zones and preserves other tiles when one tile is removed from a legacy rectangular blueprint.
- Plan overlays show the desired final terrain/furniture and a live Ready, Missing requirements, Unreachable, Invalidated, In progress, or Completed marker derived from current world state.
- Plans mode provides status/name filtering, per-plan descriptions and blockers, summary counts, map centering/highlighting, per-tile removal, and `Execute plans` through `ACT_MULTIPLE_CONSTRUCTION`.
...
- Blueprint selection used by Zone Manager remains on the existing legacy path. Existing construction JSON, IDs, `partial_con`, completion, NPC, and blueprint execution remain authoritative.
...
- Passed: strict object compilation for the Construction workspace, construction/plan adapters, multi-construction integration, and focused activity tests.
...
- Mass plan removal/replacement and other optional plan-management polish.
- Reuse of the planning geometry and viewport workflow by Zone Manager.
...
- [ ] Plan placement, duplicate no-op, explicit replacement, per-tile removal, persistence after reopening, and legacy rectangle splitting.
- [ ] Plans filtering, live status reasons, map centering/highlighting, and execution with/without available requirements.
- [ ] UI test mode distant Build and Execute plans behavior with no skills, tools, components, or light.
